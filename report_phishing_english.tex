...
\toprule
\textbf{Model Framework} & \textbf{Full (50+ Feat)} & \textbf{MI Selected (Top 15)} & \textbf{RFE Selected (Top 15)} \\
\midrule
Logistic Regression & 0.9514 & 0.6581 & 0.8698 \\
Random Forest & 0.9824 & 0.8484 & 0.9784 \\
\bottomrule
\end{tabular}
\caption{Consolidated Evaluation Metrics demonstrating strict comparative architectures across Full and Compressed feature spaces (Corrected for Data Leakage).}
\end{table}
...
\begin{tabular}{l c}
\toprule
\textbf{Document Type} & \textbf{Classifier Accuracy (Baseline RF)} \\
\midrule
Microsoft Excel (.xlsx) & 1.0000 \\
Microsoft Word (.docx) & 1.0000 \\
Adobe PDF (.pdf) & 0.9982 \\
HTML / Web (.html) & 0.9350 \\
\bottomrule
\end{tabular}
\caption{Comparison of detection accuracy across disparate file formats within the CIC Trap4Phish ecosystem.}
\end{table}
...
Foremost, the phenomenally robust classical machine learning benchmarks (Random Forest modeling reaching 98.24\%) firmly define that expertly engineered heuristic indicators contained firmly inside the CIC Trap4Phish arrays operate as phenomenally stark proxies. The data inherently provides high linear and non-linear separability; it means that simple geometries drawn strictly across structural combinations (like macro-existence flags vs. script densities) mathematically capture hostile motives beautifully. While accuracy levels approach 100\% for certain document types (Excel, Word), this is attributed to the highly discriminative nature of the Trap4Phish feature extraction methodology rather than data leakage.
